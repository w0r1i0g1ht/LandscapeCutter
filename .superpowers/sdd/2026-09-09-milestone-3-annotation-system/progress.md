...
- Base: `9b266fcd9f52b709350b319c3741baae34b0978b`
...
- Cancellation finalization: `SnipSession::cancel()` and `QSaveFile::commit()` serialize on the same per-session mutex, so cancellation cannot enter between the final flag check and atomic destination replacement. The focused race regression passed five consecutive runs after a RED check without the mutex.
...
- Post-acceptance usability repair (2026-09-12): annotation previews retain the locked-selection dim mask and border; Select is presented as Edit with tool explanations and checked state; line width and mosaic block size have labels, units, relevant-tool visibility, and stable toolbar placement; pending text commits before Copy, Save, or switching tools. Regression coverage was RED before the fix and GREEN afterward. All-target build passed, all 218 non-desktop tests passed, and desktop integration passed 4/4 in the desktop session.
